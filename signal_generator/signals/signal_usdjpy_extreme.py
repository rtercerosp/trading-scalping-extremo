...
class SignalUSDJPExtreme(ISignalGenerator):
    def __init__(self, properties: SmartMoneySignalProps, connector: PlatformConnector):
        ...
        extreme_params = getattr(config, "EXTREME_SCALPING_PARAMS", {}).get("USDJPY", {})
        self.sl_atr_mult = extreme_params.get("sl_atr_mult", 0.85)
        self.tp_atr_mult = extreme_params.get("tp_atr_mult", 2.2)
        self.min_atr_points = 18
        self.max_atr_points = 900

        self._allowed_symbols = ["USDJPY", "USDJPYc"]
    ...
    def _detect_order_block(self, bars: pd.DataFrame, signal_type: str) -> float | None:
        if len(bars) < 5:
            return None
        if signal_type == "BUY":
            for i in range(len(bars) - 3, max(len(bars) - 16, -1), -1):
    ...
    def _near_psychological_level(self, price: float, point: float) -> bool:
        if point <= 0:
            return False
        rounded = round(price / (10 * point)) * 10 * point
        return abs(price - rounded) < 5 * point
