...
class SignalSmartMoneyETH(SignalSmartMoney):
    def __init__(self, properties: SmartMoneySignalProps, connector: PlatformConnector):
        ...
        self._allowed_symbols = ["ETHUSD", "ETHUSD."]
    ...
    def generate_signal(self, data_event: DataEvent, data_provider: DataProvider,
        ...
        if long_trigger:
            ...
            print(f"{Utils.dateprint()} - SMART ETH: LONG rsi={current_rsi:.2f} macd={current_macd:.5f} atr={atr_points:.2f} spread={spread_points:.1f} pts")
            return SignalEvent(
                symbol=symbol,
                signal="BUY",
                target_order="MARKET",
        ...
        if short_trigger:
            ...
            print(f"{Utils.dateprint()} - SMART ETH: SHORT rsi={current_rsi:.2f} macd={current_macd:.5f} atr={atr_points:.2f} spread={spread_points:.1f} pts")
            return SignalEvent(
                symbol=symbol,
                signal="SELL",
                target_order="MARKET",
