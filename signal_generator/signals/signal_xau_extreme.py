...
class SignalXAUExtreme(ISignalGenerator):
    def __init__(self, properties: SmartMoneySignalProps, connector: PlatformConnector):
        ...
        extreme_params = getattr(config, "EXTREME_SCALPING_PARAMS", {}).get("XAUUSD", {})
        self.sl_atr_mult = extreme_params.get("sl_atr_mult", 0.65)
        self.tp_atr_mult = extreme_params.get("tp_atr_mult", 4.0)
        self.min_atr_points = 50
        self.max_atr_points = 2500

        self._allowed_symbols = ["XAUUSD", "XAUUSDc"]
    ...
    def _detect_order_block(self, bars: pd.DataFrame, signal_type: str) -> float | None:
        if len(bars) < 5:
            return None
        if signal_type == "BUY":
            for i in range(len(bars) - 3, max(len(bars) - 20, -1), -1):
    ...
    def _near_psychological_level(self, price: float, point: float) -> bool:
        if point <= 0:
            return False
        rounded = round(price / (100 * point)) * 100 * point
        return abs(price - rounded) < 50 * point
    ...
    def generate_signal(self, data_event: DataEvent, data_provider: DataProvider,
        ...
        trend_bars = data_provider.get_latest_closed_bars(symbol, self.trend_timeframe, 60)
        entry_bars = data_provider.get_latest_closed_bars(symbol, self.entry_timeframe, 60)
        ...
        near_psych = self._near_psychological_level(ask_price if trend_is_bullish else bid_price, symbol_info.point)

        min_stop_points = max(getattr(symbol_info, 'trade_stops_level', 0), 0) + 10
        ...
        if trend_is_bullish and long_trigger:
            ...
            return SignalEvent(
                ...
                tp1=tp1,
                tp2=tp2,
                risk_pct_override=getattr(config, "EXTREME_SCALPING_PARAMS", {}).get(normalize_symbol(symbol), {}).get("risk_pct", 0.008),
            )

        if trend_is_bearish and short_trigger:
