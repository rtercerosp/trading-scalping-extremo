...
class SignalETHStructureBreakout(ISignalGenerator):
    ...
    def generate_signal(self, data_event: DataEvent, data_provider: DataProvider,
        ...
        if trend_bars.empty or entry_bars.empty or len(entry_bars) < self.breakout_lookback + 1:
            print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: datos insuficientes trend={trend_bars.empty} entry={entry_bars.empty} len={len(entry_bars)}")
            return None

        symbol_info = self.connector.get_symbol_info(symbol)
        if symbol_info is None:
            print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: sin symbol_info")
            return None
        ...
        if not trend_is_bullish and not trend_is_bearish:
            print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: sin tendencia clara 15min bullish={trend_is_bullish} bearish={trend_is_bearish}")
            return None

        atr_series = self._atr(entry_bars, self.atr_period)
        current_atr = atr_series.iloc[-1]
        if pd.isna(current_atr) or current_atr <= 0:
            print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: ATR inválido")
            return None
        atr_points = current_atr / symbol_info.point
        if not np.isfinite(atr_points) or atr_points <= 0 or atr_points < self.min_atr_points:
            print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: ATR pts={atr_points:.2f} debajo de {self.min_atr_points}")
            return None
        ...
        if ask_price is None or bid_price is None:
            print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: sin ask/bid")
            return None

        spread = ask_price - bid_price
        print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: {symbol} ask={ask_price} bid={bid_price} spread={spread:.2f} point={symbol_info.point}")
        ...
        print(f"{Utils.dateprint()} - SIGNAL ETH STRUCT: sin trigger close={current_close} prev={prev_close} rhigh={recent_high} rlow={recent_low} rsi={current_rsi:.2f} vol_ok={volume_ok}")
        return None
