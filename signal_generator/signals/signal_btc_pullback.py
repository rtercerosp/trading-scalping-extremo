...
class SignalBTCPullback(ISignalGenerator):
    ...
    def generate_signal(self, data_event: DataEvent, data_provider: DataProvider,
        ...
        if symbol_info is None:
            print(f"{Utils.dateprint()} - SIGNAL BTC: Sin symbol_info para {symbol}")
            return None
        print(f"{Utils.dateprint()} - SIGNAL BTC: {symbol} point={symbol_info.point} trade_stops_level={symbol_info.trade_stops_level}")

        atr_series = self._atr(entry_bars, self.atr_period)
        current_atr = atr_series.iloc[-1]
        if pd.isna(current_atr) or current_atr <= 0:
            print(f"{Utils.dateprint()} - SIGNAL BTC: ATR inválido {current_atr} para {symbol}")
            return None

        atr_points = current_atr / symbol_info.point
        if not np.isfinite(atr_points) or atr_points <= 0:
            print(f"{Utils.dateprint()} - SIGNAL BTC: ATR puntos inválido {atr_points} para {symbol}")
            return None
        ...
        if trend_is_bullish and long_trigger and ask_price is not None:
            ...
            tp2 = tp
            print(f"{Utils.dateprint()} - SIGNAL BTC: LONG sl_dist={sl_distance_points:.2f} sl={sl} tp={tp} ask={ask_price}")
            return SignalEvent(
                symbol=symbol,
                signal="BUY",
                target_order="MARKET",
        ...
        if trend_is_bearish and short_trigger and bid_price is not None:
            ...
            tp2 = tp
            print(f"{Utils.dateprint()} - SIGNAL BTC: SHORT sl_dist={sl_distance_points:.2f} sl={sl} tp={tp} bid={bid_price}")
            return SignalEvent(
                symbol=symbol,
                signal="SELL",
                target_order="MARKET",
