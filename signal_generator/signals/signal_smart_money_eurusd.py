...
class SignalSmartMoneyEURUSD(SignalSmartMoney):
    ...
    def generate_signal(self, data_event: DataEvent, data_provider: DataProvider,
        ...
        if long_trigger:
            ...
            print(f"{Utils.dateprint()} - SMART EURUSD: LONG rsi={current_rsi:.2f} macd={current_macd:.5f} atr={atr_points:.2f} spread={spread_points:.1f} pts")
            return SignalEvent(
                symbol=symbol,
                signal="BUY",
                target_order="MARKET",
        ...
        if short_trigger:
            ...
            print(f"{Utils.dateprint()} - SMART EURUSD: SHORT rsi={current_rsi:.2f} macd={current_macd:.5f} atr={atr_points:.2f} spread={spread_points:.1f} pts")
            return SignalEvent(
                symbol=symbol,
                signal="SELL",
                target_order="MARKET",
